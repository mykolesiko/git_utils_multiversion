...
def compute_mean_roc(rocs,
    ...
    if multi == None:
        ...
        ret_mean = StructContainer(fpr=fpr_mean, tpr=tpr_mean)
        ret_mean["inv"] = False
        ret_mean["auc"] = np.trapz(x=fpr_mean, y=tpr_mean)
...
def compute_roc_bootstrap(X, y, pos_label = 1,
                          objective=_DEFAULT_OBJECTIVE,
                          auto_flip=False,
                          n_bootstrap=100,
                          n_samples = None,
    ...
    for _ in range(n_bootstrap):
        ...
        if len(np.unique(y_boot)) < k:
            ...
            continue
        if multi != None:
            ret = compute_roc_multi(y_score=x_boot,
                                    y_true=y_boot,
                                    multi = multi,
...
def compute_roc_multi(y_score, y_true, num_classes_or_labels,
                      multi = "Labels",
                      objective=_DEFAULT_OBJECTIVE,
                      auto_flip=False):
    rocs = []
    if multi == "Classes":
        ...
        if len(classes) != num_classes_or_labels:
            raise(ValueError, "n_classes is not equel to actual num classes")
        if classes != list(range(num_classes_or_labels)):
            raise(ValueError, "labels are not in format 0 - (num classes-1)")
        for label in classes:
              y_true_bin = [1 if x == label else 0 for x in y_true]
              #print(y_true_bin)  
              y_score_bin = y_score[:, label]
              #print(y_score_bin)  
              roc = compute_roc(y_score_bin, y_true_bin, 
                    objective=_DEFAULT_OBJECTIVE, auto_flip =  auto_flip)
              rocs.append(roc)
    elif multi == "Labels":
        if y_score.shape != y_true.shape:
            raise(ValueError, "size of ground truth is not the same as predicted")
        if y_true.shape[1] != num_classes_or_labels:
            raise(ValueError, "number of labels is not equel to actual num_classes_or_labels")
        for label in range(num_classes_or_labels):
          ...
          #print(roc.fpr, roc.tpr)
    # First aggregate all false positive rates
    all_fpr = np.unique(np.concatenate([rocs[i].fpr for i in range(num_classes_or_labels)]))

    # Then interpolate all ROC curves at this points
    mean_tpr = np.zeros_like(all_fpr)
    for i in range(num_classes_or_labels):
        tpr_new = np.interp(all_fpr, rocs[i].fpr, rocs[i].tpr)
        #print(len(tpr_new), len(all_fpr))
        #print(tpr_new)
        #print(all_fpr)
        mean_tpr += tpr_new

    # Finally average it and compute AUC
    mean_tpr /= num_classes_or_labels
    #print(mean_tpr)
    #print(all_fpr)
    
    all_fpr = np.insert(all_fpr, 0, 0)
    mean_tpr = np.insert(mean_tpr, 0, 0)
    #print(mean_tpr)
    #print(all_fpr)
    auc1 = auc(all_fpr, mean_tpr)
    flipped = False
    roc = StructContainer(fpr=all_fpr,
                             tpr=mean_tpr,
                             thr=None,
                             auc=auc1,
                             opd=None,
...
def calculate_bootstrap_auc_way2(y_score, y_true, n_bootstrap, multi, num_classes_or_labels):
    stds = []
    means = []
    if multi == "Classes":
        classes = list(set(y_true))
        ...
        for label in classes:
              ...
              y_score_bin = y_score[:, label]
              #print(y_score_bin)  
              roc = compute_roc_bootstrap(y_score_bin, y_true_bin, 
                    objective=_DEFAULT_OBJECTIVE, auto_flip =  False)#auto_flip)
              #print(roc)
              std = roc["auc_std"]
              mean = roc["auc_mean"]
              stds.append(std)
        ...
        for label in range(num_classes_or_labels):
            ...
            y_score_bin = y_score[:, label]
            roc = compute_roc_bootstrap(y_score_bin, y_true_bin, 
                    objective=_DEFAULT_OBJECTIVE, auto_flip =  False)
            std = roc["auc_std"]
            mean = roc["auc_mean"]
            stds.append(std)
            means.append(mean)       
    print(stds, means)                 
    std_sum = np.sqrt(np.sum(np.array(stds)**2))/num_classes_or_labels#/np.sqrt(n_bootstrap)      
